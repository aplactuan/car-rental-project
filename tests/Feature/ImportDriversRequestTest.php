...
beforeEach(function () {
    Route::post('/test-driver-import', function (ImportDriversRequest $request) {
...
describe('guest user', function () {
    test('it cannot upload a csv if user is not logged in', function () {
        $this->postJson('/test-driver-import')->assertUnauthorized();
    });
});
...
describe('authenticated user', function () {
    ...
    test('it rejects a request with no file', function () {
        $this->postJson('/test-driver-import')
            ->assertUnprocessable()
    ...
    test('it rejects a file exceeding 10mb', function () {
        $this->postJson('/test-driver-import', [
            'file' => UploadedFile::fake()->create('drivers.csv', 11000, 'text/csv'),
        ])
            ->assertUnprocessable()
            ->assertJsonPath('errors.0.source.pointer', '/data/attributes/file');
    });
    ...
    test('it accepts a valid csv file', function () {
        $this->postJson('/test-driver-import', [
            'file' => UploadedFile::fake()->create('drivers.csv', 100, 'text/csv'),
        ])
            ->assertOk();
    });
    ...
    test('it accepts a txt file', function () {
        $this->postJson('/test-driver-import', [
            'file' => UploadedFile::fake()->create('drivers.txt', 100, 'text/plain'),
        ])
            ->assertOk();
    });
});
